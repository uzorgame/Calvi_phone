# Додає в проєкт Xcode ціль годинникового застосунку і ускладнення на циферблаті.
#
...
# із завантаження. Розширення всередині годинника підлягає тому самому.
...
# Те, що спільне для обох годинникових цілей: платформа, сімейство, версії.
def watch_settings(target, team, version, build)
  target.build_configurations.each do |config|
...
# plist цілі мають збігатися з ними.
def add_fonts(group, target)
  fonts = Dir.glob(File.join(ROOT, 'assets', 'fonts', 'Onest-*.ttf')).sort.map { |f| group.new_reference(f) }
  target.add_resources(fonts)
end

# Спільний код застосунку і ускладнення: залишок дня в ланцюжку ключів. Один
# файл в обох цілях, щоб те, що пише одна, читала друга тим самим кодом.
def add_shared(group, target)
  refs = Dir.glob(File.join(ROOT, 'ios', 'Shared', '*.swift')).sort.map { |f| group.new_reference(f) }
...
if watch
  ...
  add_fonts(group, watch)

  watch_settings(watch, team, version, build)
...
if widget
  stamp(widget, version, build)
  puts "#{WIDGET}: ціль уже є, оновив версію #{version} (#{build})"
else
  # Ускладнення на циферблаті: розширення WidgetKit всередині годинникового
  # застосунку. Своя ціль, свій ідентифікатор, свій профіль.
  widget = project.new_target(:app_extension, WIDGET, :watchos, '10.0', nil, :swift)

  group = project.new_group(WIDGET, WIDGET.to_s)
  Dir.glob(File.join(ROOT, 'ios', WIDGET, '*.swift')).sort.each do |file|
    ref = group.new_reference(file)
    widget.add_file_references([ref])
  end
  group.new_reference(File.join(ROOT, 'ios', WIDGET, 'Info.plist'))
  add_shared(group, widget)
  add_fonts(group, widget)

  watch_settings(widget, team, version, build)
  ...
  # Розширення їде всередині годинникового застосунку, у теці PlugIns: так само,
  # як Xcode робить це сам, коли додає ціль ускладнення.
  ...
  puts "#{WIDGET}: ціль додано, #{WIDGET_ID}, версія #{version} (#{build})"
end
