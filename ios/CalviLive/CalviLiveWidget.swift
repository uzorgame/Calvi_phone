...
struct CalviLiveWidget: Widget {
  var body: some WidgetConfiguration {
    ActivityConfiguration(for: CalviLiveAttributes.self) { context in
       ...
       * Тому колір той самий, що в самого острівця: обведення перестає бути
       * окремою річчю і стає його краєм. */
      .keylineTint(ink)
    }
  }
}
...
struct LockScreenView: View {
  ...
  var body: some View {
    ...
    /* Своє тло, а не тільки прохання до системи.
     *
     * `activityBackgroundTint` це побажання для замкненого екрана, і на інші
     * кадри воно не поширюється нічим. Кадри переходу малюються поверх того, що
     * на екрані, тому картка мусить бути непрозорою сама по собі, а не за
     * чиєюсь ласкою. Заокруглення і межі лишаються системі: вона обрізає картку
     * по своїй формі, і малювати свою було б другою рамкою поверх її. */
