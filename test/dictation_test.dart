...
void main() {
  ...
  /* Яку мову віддають двигуну.
   *
   * Перелік мов від телефона описує завантажені словники, а не те, що двигун
   * уміє: на Pixel із системною українською в ньому немає ні польської, ні
   * української. Доти ми в такому випадку не передавали нічого, і двигун брав
   * мову телефона. Польський застосунок від цього диктував в український
   * двигун, а Нора відповідала українською на сказане польською. */
  group('вибір мови розпізнавання', () {
    test('точний збіг береться як є', () {
      expect(Dictation.pick(['uk_UA', 'en_US'], 'en-US'), 'en-US');
    });

    test('підкреслення від телефона не заважає збігу', () {
      expect(Dictation.pick(['pl_PL'], 'pl-PL'), 'pl-PL');
    });

    test('родич тієї ж мови годиться, коли точного немає', () {
      expect(Dictation.pick(['pt_PT'], 'pt-BR'), 'pt-PT');
    });

    test('мови немає в переліку, і вона все одно доходить до двигуна', () {
      expect(Dictation.pick(['uk_UA', 'en_US'], 'pl-PL'), 'pl-PL');
    });

    test('порожній перелік не забирає мову', () {
      expect(Dictation.pick(const [], 'de-DE'), 'de-DE');
    });
