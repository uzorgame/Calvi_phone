...
void main() {
  ...
  /* Мови, які пишуться кирилицею. Перевірки на чуже письмо питають саме цей
     перелік, а не одну українську: кириличних мов у застосунку три. */
  const cyrillicLangs = {'uk', 'ru', 'be'};
   ...
   *
   * Кириличних мов у нас тепер три, тому перевірка не про одну мову, а про
   * письмо: латинська мова не має нести кирилиці взагалі. Зворотної перевірки
   * тут немає навмисно, бо латиниця в кириличному рядку буває законною:
   * «Calvi», «Google», «HIIT» і «E621» однакові в усіх мовах. */
  test('латинська мова не несе кирилиці', () {
    for (final lang in langs) {
      if (cyrillicLangs.contains(lang)) continue;
      final bad = <String>[];
  ...
  test('назви алергенів латинською мовою без кирилиці', () {
    final bad = <String>[];
    for (final a in allergens) {
      for (final lang in langs) {
        if (cyrillicLangs.contains(lang)) continue;
  ...
  test('дата словами зібрана за правилами своєї мови', () {
    const want = {
      ...
      'cs': '2. září',
      /* Російська і білоруська, як українська, ставлять місяць у родовому і
         обходяться без крапки після числа. Назви місяців у них при цьому
         власні: «сентября» і «верасня» це різні слова, а не варіанти одного. */
      'ru': '2 сентября',
      'be': '2 верасня',
    };
    for (final lang in langs) {
