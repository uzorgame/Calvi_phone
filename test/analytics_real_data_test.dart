...
void main() {
  ...
  testWidgets('записане в базу доходить до середнього БЖВ', (tester) async {
    ...
    /* Один записаний день: середнє за день дорівнює самому дню. Числа мають
       ...
       вигадувало. */
    // Число і одиниця живуть в одному Text.rich, тому шукається по всьому
    // рядку, а не по окремому віджету.
    expect(
      find.textContaining('14 г', findRichText: true),
      findsWidgets,
      reason: 'білок не дійшов з бази',
    );
    expect(
      find.textContaining('17 г', findRichText: true),
      findsWidgets,
