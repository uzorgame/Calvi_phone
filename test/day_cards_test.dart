...
void main() {
  ...
  /* Велосипед і біг описують звичайну людину, а не спортсмена.
   *
   * Пін, а не прикраса. Ці два числа вже одного разу стояли на спортивних
   * темпах (7.5 і 9.8), і година катання давала 670 ккал замість трьохсот з
   * чимось. Живий користувач це помітив і поскаржився. Завищене спалене
   * дописується до денної норми, тобто помилка тут іде просто в дозвіл
   * зʼїсти зайве, і мовчки. */
  testWidgets('коефіцієнти велосипеда і бігу лишаються побутовими', (tester) async {
    expect(activityFor('bike')?.met, 4.5, reason: 'велосипед це їзда містом, не гонка');
    expect(activityFor('run')?.met, 8, reason: 'біг це близько 8 км/год, не марафон');

    // Година спокійної їзди для людини на 85 кг: 4.5 × 3.5 × 85 / 200 × 60.
    expect(burnEstimate(4.5, 60, weightKg: 85), 400);
    // Півгодини звичайного бігу для неї ж.
    expect(burnEstimate(8, 30, weightKg: 85), 355);
  });
