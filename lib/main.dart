...
class _CalviAppState extends State<CalviApp> with WidgetsBindingObserver {
  ...
  void _finishStart(StartDraft draft) {
    ...
    setState(() {
      ...
      /* Перше, що людина бачить у дні, це піднята штора чату з карткою Нори.
       *
       * Порожній день пояснити нічим: у ньому немає жодного запису, і всі
       * числа на ньому нулі. Показати замість цього розмову означає почати з
       * того, заради чого застосунок і встановили, і показати це рухом, а не
       * підказкою поверх екрана.
       *
       * Нового тексту тут не зʼявляється: картка порожньої розмови стоїть у
       * чаті від початку, і вона ж вітається. Змінюється тільки те, чи людина
       * побачить її сама, чи муситиме здогадатись натиснути на рядок унизу.
       *
       * Тим, хто увійшов у наявний акаунт, штора не піднімається: анкети вони
       * не проходять і сюди не потрапляють.
       *
       * Прапорець, а не дія: підняти має день, бо чат живе всередині нього. */
      _greet = true;
    });
   ...
   *
   * Той, хто натиснув «У мене вже є акаунт», анкети не проходив, і всі поля
   * «Старту» стоять на заводських значеннях: чоловік, 26 років, 178 см, 80 кг.
   * Зберегти цей чернетковий профіль означало б відправити його на сервер і
   * затерти справжній: обмін віддає перевагу тому боку, який щойно змінився, а
   * змінився б саме телефон. Людина заходила по свої цілі, а отримала б чужі.
   *
   * Тому тут не зберігається нічого. Робиться обмін, і профіль забирається з
   * акаунта таким, яким він там лежить.
   *
   * `false` означає «в акаунті профілю немає»: так буває, коли вхід відбувся, а
   * «Старт» на тому пристрої так і не пройшли. Тоді анкету доведеться пройти,
   * і «Старт» сам поверне до першого питання.
   */
  Future<bool> _accountReady() async {
    /* Сам вхід уже зробив повний обмін і привіз профіль, якщо той є. Цей такт
       додається на випадок, коли обмін після входу не дожив до кінця, і його
       невдача нічого не вирішує: далі однаково дивимось, чи профіль на місці. */
    try {
      await _sync?.now();
    } catch (_) {}
  ...
  Future<void> _eraseAll() async {
    ...
    /* Привітання гасне разом із даними. Екран дня народжується заново, а з ним
       і чат, і незнятий прапорець підняв би штору вдруге, ніби застосунок
       щойно поставили. Стерти щоденник це не почати спочатку. */
    setState(() {
      _era++;
      _greet = false;
      _hail = false;
  ...
  /// Перший запуск щойно закінчився: день має підняти штору чату сам.
  bool _greet = false;
        ...
        home: Builder(
          builder: (context) {
            ...
            return _start(switch (_onboarding) {
              ...
              true => StartScreen(onFinish: _finishStart, onSignedIn: _accountReady),
              false => TodayScreen(
