...
class FoodRepository {
  ...
  Future<bool> enrich(String mealId, String text, {double? grams}) async {
    ...
    await db.diaryDao.applyFood(
      ...
      grams: n.grams,
      /* Другий рівень їде разом із рештою, а не чекає синхронізації.
       *
       * Цей рядок уже брудний і поїде на сервер сам: лишити пʼятірку порожньою
       * означало б записати туди порожнечу поверх того, що довідник щойно
       * сказав, і більше її ніхто не заповнить. */
      nutrients: n.tier,
    );
    return true;
  }
