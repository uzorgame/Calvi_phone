...
/// Стать, і тільки заради формули норми.
///
/// Вибрати можна два: Міффлін-Сан Жеор має рівно два коефіцієнти, і третій
/// варіант однаково рахувався б за чоловічий. Вибір, який ні на що не впливає,
/// це вибір, який вводить в оману.
///
/// [x] лишається в переліку, хоч його більше ніде не пропонують: він стоїть у
/// записах, зроблених до цієї зміни, і викинути його означало б, що профіль
/// такої людини перестане читатись зовсім. Скрізь у розрахунках він поводиться
/// як [m], бо так поводився й раніше.
enum Sex { m, f, x }
...
/// Скільки норми віддається білку в найгіршому випадку.
///
/// Не смак і не мода, а межа, за якою розкладка перестає бути можливою: жири
/// беруть 28%, і якби білок міг узяти більше за 35%, вуглеводам лишалось би
/// менше за нуль.
const _proteinCap = 0.35;

/// Норма, розкладена на білок, жири і вуглеводи.
///
/// Білок за вагою, жири часткою від норми, а вуглеводи це решта. Порядок саме
/// такий, бо білок і жири це те, чого має бути не менше, а вуглеводи те, що
/// лишається.
///
/// **Стеля на білок не косметична.** Без неї 1.7 грама на кілограм ваги важкої
/// людини перевищували всю норму: жінка 180 кг із нормою на підлозі отримувала
/// 306 г білка, тобто 1224 ккал самого білка при нормі 1200, і вуглеводів
/// виходило мінус вісімдесят девʼять. Відʼємна ціль зберігалась у профіль і
/// їхала на картку дня. На переборі всіх відповідей, які анкета дозволяє, таких
/// випадків було 958 із 280 800.
///
/// На звичайному профілі стеля не спрацьовує: у людини 80 кг із нормою 2220
/// білок займає 24.5%. Вона вмикається рівно там, де без неї ламалось.
///
/// Вага береться на старті цілі, як і сама норма: рахувати білок від живої, а
/// калорії від старої означало б розкладати одне число за міркою іншого.
({int protein, int fat, int carbs}) macrosFor(SettingsState s) {
  final kcal = calcKcal(s);

  final byWeight = s.goalStartKg * 1.7;
  final ceiling = kcal * _proteinCap / 4;
  final protein = (byWeight < ceiling ? byWeight : ceiling).round();
 ...
 *
 * Спершу англійська і німецька, далі решта за абеткою власних назв, а
 * кирилиця в кінці: так само стоять списки мов у застосунках, які цим живуть.
 * Українська більше не друга: список читає той, хто шукає свою мову в чужому
 * інтерфейсі, і йому потрібна абетка, а не наш порядок появи.
 *
 * «Мови пристрою» в списку немає, хоч [Lang.system] і лишається значенням.
 * Рядок про режим серед восьми справжніх мов читався як девʼята мова, і його
 * вибирали, не розуміючи, що обрали. Сам режим нікуди не подівся: він і є
 * замовчуванням до першого вибору, тому німецький телефон відкриває застосунок
 * німецькою без жодного дотику. Список лише показує, якою мовою застосунок
...
const langOptions = <Lang>[
  ...
  Lang.pl,
  Lang.pt,
  /* Кирилиця в кінці. Між собою вона теж за абеткою, але російська стоїть
     останньою навмисно, поза абеткою: це рішення власника, а не недогляд, і
     переставляти її «щоб було за алфавітом» не треба. */
  Lang.be,
  Lang.uk,
  Lang.ru,
];
